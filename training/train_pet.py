...
from aes2.utils.savor import save_checkpoints, load_lm_head_state

from pet.models import AESRegressionModel
    ...
    def evaluate(self):
        ...
        for x, y in tqdm(self.val_dataloader, total=len(self.val_dataloader)):
            ...
            if self.config.model.task_type == "regression":
                logits = logits.cpu().detach().numpy() * 5 + 1
            else:
        ...
        oof_pred["score"] = oof_pred["score"] * 5 + 1
        if config.model.task_type == "regression":
            ...
            oof_pred.loc[:, [f"pred{i}" for i in range(6)]] = prediction
            oof_pred.loc[:, "pred"] = np.argmax(prediction, axis=-1) + 1

        print(oof_pred.head(20))
        score = competition_score(oof_pred["pred"], oof_pred["score"].values)
        ...
        for epoch in range(config.train.epochs):
            ...
            if self.val_dataloader is not None:
                ...
                if best_score > self.best_score:
                    ...
                    oof_pred.to_csv(os.path.join(self.config.model.save_path, f"pred_df_fold{fold}.csv"), index=False)
...
def tokenize(example, tokenizer, config, max_length=1024):
    ...
    truncation = True
    d = dict(mask_token=tokenizer.mask_token)
    if "topic" in config.dataset.label_text_keys:
        d["topic"] = "Kaggle only" if not example["is_pc2"] else "Persuade"
    text = config.dataset.label_text.format(**d)
    text += tokenizer.sep_token + full_text
    encoding = tokenizer(text, return_token_type_ids=False, truncation=truncation, max_length=max_length)
    mask_token_indexes = []
    for i, input_id in enumerate(encoding["input_ids"]):
        if input_id == tokenizer.mask_token_id:
            mask_token_indexes.append(i)
            break
    ...
    return {
        ...
        "mask_token_indexes": mask_token_indexes[0]
    }
...
if __name__ == '__main__':
    ...
    if config.train.loss == "ce" and config.model.n_labels == 6:
    ...
    tokenizer.add_tokens([AddedToken("\n", normalized=False)])
    # positive_tokens = ["good", "awesome", "6", "Good", "strong", "accurate", "excellent"]
    positive_tokens = [
        "Excellent", "Good", "A", "6",
    ...
    if config.model.task_type == "classification":
        positive_tokens = ["1", "2", "3", "4", "5", "6"]
    config.model.positive_token_ids = tokenizer.convert_tokens_to_ids(positive_tokens)
    config.model.negative_token_ids = tokenizer.convert_tokens_to_ids(negative_tokens)
    print(f"<<< positive token ids: {config.model.positive_token_ids}")
    print(f"<<< positive tokens: {tokenizer.convert_ids_to_tokens(config.model.positive_token_ids)}")
    print(f"<<< negative token ids: {config.model.negative_token_ids}")
    print(f"<<< negative tokens: {tokenizer.convert_ids_to_tokens(config.model.negative_token_ids)}")
    ...
    keep_columns = ["input_ids", "attention_mask", "labels", "mask_token_indexes"]
    ...
    # safe_tensor_file = os.path.join(config.model.path, "model.safetensors")
    # if os.path.isfile(safe_tensor_file):
    #     print("<<< loading lm head state dict")
    #     state_dict = load_lm_head_state(safe_tensor_file)
    #     if state_dict:
    ...
    if config.train.save_last_checkpoint and not config.train.fullfit:
        ...
        torch.save(model.half().state_dict(), os.path.join(save_to, f"model_fold_full_seed{config.train.seed}.path"))
