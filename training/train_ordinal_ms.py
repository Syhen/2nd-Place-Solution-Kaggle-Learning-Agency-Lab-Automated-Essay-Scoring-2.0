...
from ordinal_multi_scale.models import AESRegressionModel
from ordinal_multi_scale.yao_datasets import AES2Dataset, NEW_TOKENS
from ordinal_multi_scale.collators import CustomDataCollatorWithPadding
...
class Trainer(object):
    ...
    def train_one_epoch(self):
        ...
        for x in tqdm_obj:
            ...
            if self.config.train.hyper_loss_factor:
    ...
    def evaluate(self):
        ...
        for x in tqdm(self.val_dataloader, total=len(self.val_dataloader)):
            y = x["labels"]
            logits = self.model(x)
            ...
            losses.update(loss.item(), y.shape[0])
            logits = logits.sigmoid().sum(dim=1).cpu().detach().numpy() + 1
            if self.config.train.hyper_loss_factor:
                logits_clf = logits_clf.softmax(-1).cpu().detach().numpy()
                logits_clf = logits_clf @ arr_labels
                logits = logits + logits_clf
                logits /= 2
...
if __name__ == '__main__':
    ...
    if config.train.stage == 1:
        ...
        # sample一部分留给stage2. sample的数量为验证集中#KO * 1/4, 1/2, 1/1
        ...
        # 合并stage1 中 sampled的部分
        df_train = df_train[df_train["is_pc2"] == 0].reset_index(drop=True)
        sample_ratio = config.train.get("sample_ratio", -1)
        if sample_ratio != -1:
            df_sampled = pd.read_csv(f"ordinal_sampled_fold{fold}.csv")
            print(f"<<< append {len(df_sampled)} samples to train, now have {len(df_val) + len(df_sampled)} samples in train.")
            df_train = pd.concat([
                df_train,
                df_sampled
    ...
    ds_train = ds.get_dataset(df_train)
    ds_val = ds.get_dataset(df_val)
    ...
    keep_columns = [
        ...
        "span_head_idxs", "span_tail_idxs", "sentence_head_idxs", "sentence_tail_idxs",
        "labels", "is_pc2"
    ]
    ...
    collate_fn = CustomDataCollatorWithPadding(max_length=max_length, tokenizer=tokenizer)
    ...
    collate_fn = CustomDataCollatorWithPadding(max_length=config.train.validate_max_length, tokenizer=tokenizer)
    ...
    if config.train.use_wandb:
        ...
        wandb.init(config=OmegaConf.to_container(config), project="AES2", name=name, dir="./logs/",
                   group=config.train.exp_name, tags=["ordinal-regression", "two-stage"])
    try:
        trainer.fit(config)
        # trainer.evaluate()
    ...
    elif config.train.fullfit and config.train.stage == 1:
        save_name = f"model_full_stage1_seed{config.train.seed}.pth"
        torch.save(model.half().state_dict(), os.path.join(save_to, save_name))
